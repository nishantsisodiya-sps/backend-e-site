...
exports.sendPasswordResetEmail = async (req, res) => {
  try {
    ...
    const resetLink = `https://fullstck-ecommerce-mhfs.vercel.app/reset-password/?reset_token=${resetToken}`;
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // Decrypt the email
    const decryptedToken = jwt.verify(resetToken, process.env.JWT_SECRET);
    const { email, name } = decryptedToken;

    // Find the user or seller based on the decrypted email
    const user = await User.findOne({ email });
    const seller = await Seller.findOne({ email });
